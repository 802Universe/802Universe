...
function renderThreat() {
  ...
  host.innerHTML =
    `<div class="conv-hint">${rows.length.toLocaleString()} external endpoint${rows.length === 1 ? "" : "s"} · country via DB-IP · click to filter</div>
    ...
    rows.map(([ip, e]) => {
      ...
      return `<tr data-filter="${escAttr("ip.addr==" + ip)}"><td class="mono">${esc(ip)}</td>
       <td class="mono" style="font-size:10px">${country}</td><td class="mono" style="color:var(--muted)">—</td>
