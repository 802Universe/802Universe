...
function onLoaded(file, summary) {
  ...
  state.convCache = {}; state.convInit = false; state.convType = "IPv4";
  clearDetail();
  loadPage(true);
...
function switchView(name) {
  ...
  $("#view-conversations").style.display  = name === "conversations" ? "" : "none";
  if (name === "conversations" && !state.convInit) { state.convInit = true; renderConvTabs(); loadConv(state.convType); }
}
