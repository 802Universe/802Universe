...
function onLoaded(file, summary) {
  ...
  // reset packet list, conversations & selection
  state.summary = summary;
  state.rendered = { overview: true, packets: true };
  state.filter = ""; $("#filterInput").value = "";
  state.convCache = {}; state.convInit = false; state.convType = "IPv4"; state.streamType = "TCP";
  clearDetail();
  loadPage(true);
...
/* ------------------------------------------------------------------ views */
const VIEWS = ["overview", "packets", "conversations", "hierarchy", "statistics", "streams", "findings", "wireless", "threat"];
...
function renderView(name) {
  switch (name) {
    ...
    case "streams":       renderStreamTabs(); loadStreamList(state.streamType); break;
...
function loadConv(type) {
  ...
  const rows = convs.map((c) => {
    const a = hasPorts ? `${c.saddr}<b>:${c.sport}</b>` : esc(c.saddr);
    const b = hasPorts ? `${c.daddr}<b>:${c.dport}</b>` : esc(c.daddr);
...
/* streams — follow TCP/UDP (verified: uppercase name + conversation filter) */
const STREAM_TYPES = ["TCP", "UDP"];
function renderStreamTabs() {
  $("#streamTabs").className = "conv-tabs stream-tabsrow";
  $("#streamTabs").innerHTML = STREAM_TYPES.map(t => `<button class="conv-tab ${t === state.streamType ? "active" : ""}" data-st="${t}">${t}</button>`).join("");
  $$("#streamTabs .conv-tab").forEach(b => b.addEventListener("click", () => {
    state.streamType = b.dataset.st;
    $$("#streamTabs .conv-tab").forEach(x => x.classList.toggle("active", x === b));
    loadStreamList(state.streamType);
  }));
}
function loadStreamList(type) {
  let convs;
  try { const r = state.wg.tap({ tap0: `conv:${type}` }); convs = (r.error || !r.taps.length) ? [] : (r.taps[0].convs || []); } catch { convs = []; }
  const host = $("#streamList");
  if (!convs.length) { host.innerHTML = `<div class="conv-empty">No ${esc(type)} streams.</div>`; return; }
  host.innerHTML = convs.map(c => `<div class="stream-row" data-type="${type}" data-filter="${escAttr(c.filter)}">
     <span>${esc(c.saddr)}:<b>${esc(c.sport)}</b></span><span>${esc(c.daddr)}:<b>${esc(c.dport)}</b></span>
     <span class="num">${(c.tx_frames_total + c.rx_frames_total).toLocaleString()} pkts</span>
     <span class="num">${fmtBytes(c.tx_bytes_total + c.rx_bytes_total)}</span></div>`).join("");
  $$("#streamList .stream-row").forEach(el => el.addEventListener("click", () => {
    $$("#streamList .stream-row").forEach(x => x.classList.remove("sel")); el.classList.add("sel");
    followStream(el.dataset.type, el.dataset.filter);
  }));
}
function followStream(type, filter) {
  const host = $("#streamPayload");
  let f;
  try { f = state.wg.follow(type, filter); } catch { host.innerHTML = `<div class="conv-empty">Couldn't follow this stream.</div>`; return; }
  const n = f.payloads ? f.payloads.size() : 0;
  if (!n) { host.innerHTML = `<div class="conv-empty">No reassembled payload in this stream.</div>`; return; }
  const client = `${f.chost}:${f.cport}`, server = `${f.shost}:${f.sport}`, CAP = 300;
  let chunks = "";
  for (let i = 0; i < Math.min(n, CAP); i++) { const p = f.payloads.get(i); chunks += `<div class="chunk ${p.server ? "s2c" : "c2s"}">${esc(b64ToText(p.data))}</div>`; }
  if (n > CAP) chunks += `<div class="conv-empty">… ${(n - CAP).toLocaleString()} more segments</div>`;
  host.innerHTML = `<div class="stream-legend">
     <span><i style="background:var(--cobalt)"></i> ${esc(client)} → ${esc(server)}</span>
     <span><i style="background:var(--signal)"></i> ${esc(server)} → ${esc(client)}</span></div>${chunks}`;
}
function b64ToText(b64) { const b = b64ToBytes(b64); let s = ""; for (let i = 0; i < b.length; i++) { const c = b[i]; s += (c === 9 || c === 10 || c === 13 || (c >= 32 && c < 127)) ? String.fromCharCode(c) : "."; } return s; }
